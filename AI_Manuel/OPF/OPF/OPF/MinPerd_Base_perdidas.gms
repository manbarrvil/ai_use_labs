** SMARTIE 3 TERMINALES CON P�RDIDAS -- MAX_GEN
*********
...
SCALAR Vmax   Tensi�n m�xima en cada nudo
       Vmin   Tensi�n m�nima en cada nudo
       V_SUBEST   tensi�n en barras de la SE
...
PARAMETER
          ...
          COS_PHI
*          r_link_pmin(n,n)   M�nimo flujo de p activa en link de continua (flujo inverso)
*          r_link_pmax(n,n)   M�ximo flujo de p activa en link de continua
*          r_link_smax        M�ximo flujo de p aparente en link de continua
          ;
FdG=4;
P(ND) = DATNUD(ND,'P')*1;
Q(ND) = DATNUD(ND,'Q')*1;
...
EQUATION     smax_n  m�xima potencia aparente de cada convertidor  ;
...
PARAMETER dLAMBDA incremento de cargabilidad
          dLAMBDAG(NG) incremento de generaci�n distribuida
          PERD perdidas en W
          V    tensiones en pu
...
PUT '- P y Q inyectada desde la subestaci�n (W y var)' /
PUT 'P: ' PSE:< /
PUT 'Q: ' QSE:< /