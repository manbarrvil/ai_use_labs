...
SCALARS
    SBASE potencia base [VA] / 100000 / ;

SCALARS
    VBASE tension base [V] /  400/ ;

* Se define el factor de carga para cada uno de los ensayos
...
COS_PHI_CONV =  0 ;        ! quitar en nuevos datos minutos
FdC=1;
X_TC = 0.000000001 ;
X_SE = 0.000000001 ;
...
DATNUD(nd, 'P') = (LOADH_DATA(nd, 'P') + LOADI_DATA(nd, 'P') ) * 1 / SBASE ;
DATNUD(nd, 'Q') = (LOADH_DATA(nd, 'Q')* 0.6197  + LOADI_DATA(nd, 'Q') * 0.2031 ) * 0 / SBASE ;

* Datos de los generadores
DATGEN(ng, 'p0') = SUM(NDGR(ng,nd), GEN_DATA(ng, nd, 'P'))*1 / SBASE  ;
DATGEN(ng, 'pmin') = SUM(NDGR(ng,nd),GEN_DATA(ng, nd, 'Pmin')) / SBASE  ;
DATGEN(ng, 'pmax') = SUM(NDGR(ng,nd),GEN_DATA(ng, nd, 'Pmax'))*1 / SBASE  ;
*DATGEN(ng, 'status') = SUM(NDGR(ng,nd),GEN_DATA(ng, nd, 'status'))   ;

* Datos de las lineas!!!! Multiplicado por 4. La red de baja tensión en el labs está conectada en serie.
* p.u. conversion
DATLIN(kl(nl, ni, nf), 'X') = LINE_DATA(nl, ni, nf, 'X') * 4 *  LINE_DATA(nl, ni, nf, 'L') *
                                 SBASE / (VBASE**2)   ;
DATLIN(kl(nl, ni, nf), 'R') = LINE_DATA(nl, ni, nf, 'R')* 4 * LINE_DATA(nl, ni, nf, 'L') *
                                 SBASE / (VBASE**2)   ;
DATLIN(kl(nl, ni, nf), 'Bsh') = LINE_DATA(nl, ni, nf, 'C') * LINE_DATA(nl, ni, nf, 'L') *
                                 2 * pi * 50 * 1e-9 / SBASE * (VBASE**2)   ;
