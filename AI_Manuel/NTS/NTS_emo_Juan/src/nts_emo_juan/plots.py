...
def plot_eigenvalue_sweep(results: dict,
    ...
    Parameters
    ----------
    results       : dict returned by :func:`nts_emo_juan.analysis.xl_sweep`
    ...
    for zeta, colour in zip(damping_cones, cone_colours):
    ...
    for i, (xl, lam) in enumerate(zip(xl_values, em_modes)):
        if np.isnan(lam.real):
            continue
        f_hz = abs(lam.imag) / (2 * np.pi)
        ax.scatter(lam.real, f_hz, color=colours[i], s=50, zorder=4,
    ...
    ax.set_title('Eigenvalue sweep — NTS §5.10.2.1 (pydae-BPS, no MPE)', fontsize=11)
